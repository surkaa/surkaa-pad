...
fn main() {
    ...
    let credentials = s3::creds::Credentials::new(Some(&akid), Some(&aks), None, None, None).unwrap();
    let bucket = Bucket::new(&bucket_name, region, credentials).unwrap().with_service("oss");
    ...
    match cmd {
        "list" => {
            ...
            let mut total = 0;
            loop {
                let (result, _) = rt.block_on(bucket.list_page(prefix.to_string(), None, token, None, None)).unwrap();
        ...
        "upload" => {
            let key = args.get(2).expect("用法: oss_tool upload <key> <file_path>");
            let path = args.get(3).expect("用法: oss_tool upload <key> <file_path>");
            let data = std::fs::read(path).expect("无法读取文件");
            let resp = rt.block_on(bucket.put_object(key, &data)).unwrap();
            println!("已上传: {} ({} bytes, HTTP {})", key, data.len(), resp.status_code());
        }

        "put" => {
            let key = args.get(2).expect("用法: oss_tool put <key> <text>");
            let text = args.get(3).expect("用法: oss_tool put <key> <text>");
            let resp = rt.block_on(bucket.put_object(key, text.as_bytes())).unwrap();
            println!("已写入: {} ({} bytes, HTTP {})", key, text.len(), resp.status_code());
        }

        "download" => {
            let key = args.get(2).expect("用法: oss_tool download <key> [output_path]");
            let response_data = rt.block_on(bucket.get_object(key)).unwrap();
            match args.get(3) {
                Some(path) => {
                    std::fs::write(path, response_data.as_slice()).expect("无法写入文件");
                    println!("已下载到: {} ({} bytes)", path, response_data.as_slice().len());
                }
                None => {
        ...
        "head" => {
            ...
            println!("  etag:          {}", result.e_tag.as_deref().unwrap_or("-"));
            println!("  content_type:  {}", result.content_type.as_deref().unwrap_or("-"));
            println!("  content_len:   {:?}", result.content_length);
            println!("  last_modified: {}", result.last_modified.as_deref().unwrap_or("-"));
        }
