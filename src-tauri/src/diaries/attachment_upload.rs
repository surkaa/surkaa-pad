...
mod tests {
    ...
    const TEST_DIARY_ID: &str = "123";

    fn test_attachment_key(attachment_id: &str) -> String {
        ObjectLocations::diary_attachment(TEST_DIARY_ID, attachment_id)
    }
    ...
    async fn local_session_writes_chunks_and_commits_real_etag() {
        ...
        let key = test_attachment_key("attachment");
        assert_eq!(los.get(&key).await.unwrap(), Some(etag));
        assert_eq!(los.get_data(&key).await.unwrap(), data);
    }
    ...
    async fn local_session_rejects_incomplete_upload_and_removes_temp_file() {
        ...
        let store = LocalStore::new(los.clone());
        let mut session = store
            .begin_attachment_upload(TEST_DIARY_ID, "attachment", 5, "application/octet-stream")
            .await
            .unwrap();
        ...
        assert!(los
            .get(&test_attachment_key("attachment"))
            .await
            .unwrap()
            .is_none());
        assert!(std::fs::read_dir(
            temp_dir
                .path()
                .join(ObjectLocations::diary_attachments_prefix(TEST_DIARY_ID))
        )
        .unwrap()
        .next()
        .is_none());
    ...
    async fn local_session_rejects_data_larger_than_declared_size() {
        ...
        let store = LocalStore::new(los.clone());
        let mut session = store
            .begin_attachment_upload(TEST_DIARY_ID, "attachment", 2, "application/octet-stream")
            .await
            .unwrap();
    ...
    async fn local_store_uploads_large_stream_with_confirmed_progress() {
        ...
        assert_eq!(etag, format!("{:X}", md5::compute(&data)));
        assert_eq!(
            los.get_data(&test_attachment_key("attachment"))
                .await
                .unwrap(),
            data
        );
    ...
    async fn remote_session_commits_same_data_and_etag_to_oss_and_cache() {
        ...
        let key = ObjectLocations::diary_attachment("8215021834823", "attachment");
        let metadata = client.get_metadata(&key).await.unwrap();
        assert_eq!(metadata.content_length, Some(total_size));
        assert_eq!(metadata.etag.as_deref(), Some(etag.as_str()));
        assert_eq!(los.get(&key).await.unwrap().as_deref(), Some(etag.as_str()));
        assert_eq!(los.get_data(&key).await.unwrap().len() as u64, total_size);
        assert!(temp_dir
            .path()
    ...
    async fn remote_session_keeps_successful_upload_when_attachment_exceeds_per_file_cache_limit() {
        ...
        let key = ObjectLocations::diary_attachment("8215021834823", "att-large");
        let metadata = client.get_metadata(&key).await.unwrap();
        assert_eq!(metadata.etag.as_deref(), Some(etag.as_str()));
    ...
    async fn remote_session_abort_removes_local_temp_and_remote_upload() {
        ...
        let store = RemoteStore::new(los.clone(), client.clone());
        let mut session = store
            .begin_attachment_upload(TEST_DIARY_ID, "attachment", 3, "application/octet-stream")
            .await
            .unwrap();
        session.write_chunk(b"123".to_vec()).await.unwrap();
        let uploads = wait_for_multipart_upload_count(&client, 1).await;
        let key = test_attachment_key("attachment");
        assert_eq!(uploads[0].0, key);
